...
var EDITOR = (function (my) {
  ...
  my.Selection = (function () {
    ...
    function getChangedContext(node, context) {
      var until = my.Core.activeEditable ? my.Core.activeEditable.obj.parent()[0] : null;
      var parents = $(node).parentsUntil(until).get();
      var html = makeContextHtml(node, parents);
      var equal = (context && node === context.node && my.Arrays.equal(context.parents, parents) && html === context.html);
      return equal ? null : {
        node: node,
        parents: parents,
        html: html
     ...
     */
      ...
      onChange: function (objectClicked, event, timeout) {
        ...
        this.updateSelectionTimeout = window.setTimeout(function () {
          var range = new my.Selection.SelectionRange(true);
          ...
          my.Selection._updateSelection(event, range);
        }, timeout || 5);
      },
      ...
      isSelectionEditable: function () {
        return (this.rangeObject.commonAncestorContainer && $(this.rangeObject.commonAncestorContainer).contentEditable());
      },
      ...
      isFloatingMenuVisible: function () {
        var visible = $(my.Selection.rangeObject.commonAncestorContainer).attr('data-aloha-floatingmenu-visible');
      ...
      _updateSelection: function (event, range) {
        ...
        this.rangeObject = range =
            range || new Selection.SelectionRange(true);
        ...
        if (range && range.startContainer && range.endContainer) {
          ...
          if (inEditable) {
            var validStartPosition = !(3 === range.startContainer.nodeType &&
                !$(range.startContainer.parentNode).contentEditable());

            var validEndPosition = !(3 === range.endContainer.nodeType &&
                !$(range.endContainer.parentNode).contentEditable());
        ...
        my.trigger('aloha-selection-changed-before', [this.rangeObject, event]);
        ...
        my.trigger('aloha-selection-changed', [this.rangeObject, event]);

        triggerSelectionContextChanged(this.rangeObject, event);

        my.trigger('aloha-selection-changed-after', [this.rangeObject, event]);
      ...
      getSelectionTree: function (rangeObject) {
        ...
        if (!rangeObject.commonAncestorContainer) {
          console.error(this, 'the rangeObject is missing the commonAncestorContainer');
          return false;
        }
        ...
        if (my.Utils.Dom.doCleanup({ 'merge': true }, rangeObject)) {
          rangeObject.update();
          rangeObject.select();
        }
      ...
      recursiveGetSelectionTree: function (rangeObject, currentObject) {
        ...
        var jQueryCurrentObject = $(currentObject),
          childCount = 0,
          that = this,
          currentElements = [];
        ...
        jQueryCurrentObject.contents().each(function (index) {
          ...
          if (rangeObject.isCollapsed() && currentObject === rangeObject.startContainer && rangeObject.startOffset == index) {
            ...
            currentElements[childCount] = new my.Selection.SelectionTree();
      ...
      isRangeObjectWithinMarkup: function (rangeObject, startOrEnd, markupObject, tagComparator, limitObject) {
        ...
        if (parents.length > 0) {
          parents.each(function () {
            ...
            if (this === limitObject) {
              console.debug(that, 'reached limit dom obj');
            ...
            if (tagComparator(this, markupObject)) {
              ...
              console.debug(that, 'reached object equal to markup');
              i++;
              returnVal[i] = this;
      ...
      standardSectionsAndGroupingContentComparator: function (domobj, markupObject) {
        if (domobj.nodeType !== 1) {
          console.debug(this, 'only element nodes (nodeType == 1) can be compared');
          return false;
        }
        if (!markupObject[0].nodeName) {
          return false;
        }
        var elemMap = this.replacingElements[domobj.nodeName.toLowerCase()];
      ...
      standardAttributesComparator: function (domobj, markupObject) {
        var classesA = my.Strings.words((domobj && domobj.className) || '');
        var classesB = my.Strings.words((markupObject.length && markupObject[0].className) || '');
        my.Arrays.sortUnique(classesA);
        my.Arrays.sortUnique(classesB);
        return my.Arrays.equal(classesA, classesB);
      },

      /**
       * method finds out, if a node is within a certain markup or not
       * @param rangeObj Aloha rangeObject
       * @param markupObject jQuery object of the markup to be applied (e.g. created with obj = $('<b></b>'); )
       * @param tagComparator method, which is used to compare the dom object and the jQuery markup object. the method must accept 2 parameters, the first is the domobj, the second is the jquery object. if no method is specified, the method this.standardTextLevelSemanticsComparator is used
      ...
      changeMarkup: function (rangeObject, markupObject, tagComparator) {
        ...
        if (this.replacingElements[tagName]) {
          ...
          if (my.Core.activeEditable) {
            newCAC = my.Core.activeEditable.obj.get(0);
          } else {
            newCAC = $('body');
          }
          ...
          if (rangeObject.isCollapsed()) {
            console.debug(this, 'early returning from applying markup because nothing is currently selected');
            return false;
          }
        }
        ...
        if (my.Core.activeEditable) {
          limitObject = my.Core.activeEditable.obj[0];
        } else {
          limitObject = $('body');
        }
        ...
        if (!markupObject.isReplacingElement && (relevantMarkupObjectsAtSelectionStart && !relevantMarkupObjectsAtSelectionEnd)) {
          console.info(this, 'markup 2 non-markup');
          this.prepareForRemoval(rangeObject.getSelectionTree(), markupObject, tagComparator);
          $(relevantMarkupObjectsAtSelectionStart).addClass('preparedForRemoval');
          ...
          console.info(this, 'markup 2 markup');
          this.prepareForRemoval(rangeObject.getSelectionTree(), markupObject, tagComparator);
          ...
          console.info(this, 'non-markup 2 markup OR with next2markup');
          ...
          if (relevantMarkupObjectBeforeSelection && relevantMarkupObjectAfterSelection) {
            extendedRangeObject = new SelectionRange(rangeObject);
            extendedRangeObject.startContainer = $(relevantMarkupObjectBeforeSelection[relevantMarkupObjectBeforeSelection.length - 1]).textNodes()[0];
            extendedRangeObject.startOffset = 0;
            extendedRangeObject.endContainer = $(relevantMarkupObjectAfterSelection[relevantMarkupObjectAfterSelection.length - 1]).textNodes().last()[0];
            ...
            console.info(this, 'double extending previous markup(previous and after selection), actually wrapping it ...');
            ...
            console.info(this, 'extending previous markup');

          } else if (relevantMarkupObjectBeforeSelection && !relevantMarkupObjectAfterSelection && relevantMarkupObjectsAtSelectionEnd) {
            extendedRangeObject = new SelectionRange(rangeObject);
            extendedRangeObject.startContainer = $(relevantMarkupObjectBeforeSelection[relevantMarkupObjectBeforeSelection.length - 1]).textNodes()[0];
            extendedRangeObject.startOffset = 0;
            extendedRangeObject.endContainer = $(relevantMarkupObjectsAtSelectionEnd[relevantMarkupObjectsAtSelectionEnd.length - 1]).textNodes().last()[0];
            extendedRangeObject.endOffset = extendedRangeObject.endContainer.length;
            extendedRangeObject.update();
            this.applyMarkup(extendedRangeObject.getSelectionTree(), rangeObject, markupObject, tagComparator);
            console.info(this, 'double extending previous markup(previous and relevant at the end), actually wrapping it ...');
            ...
            console.info(this, 'extending following markup backwards');
          ...
          console.info(this, 'non-markup 2 non-markup');
          ...
          if (isCollapsedAndEmptyOrEndBr(rangeObject)) {
            ...
            if (Engine.isEditable(rangeObject.startContainer)) {
              ...
              $(rangeObject.startContainer).after(newMarkup[0]).remove();
            } else if (Engine.isEditingHost(rangeObject.startContainer)) {
              $(rangeObject.startContainer).append(newMarkup[0]);
        ...
        if (markupObject.isReplacingElement) {
          ...
          if (backupRangeObject && backupRangeObject.startContainer.className && backupRangeObject.startContainer.className.indexOf('preparedForRemoval') > -1) {
            //var parentElement = $(backupRangeObject.startContainer).closest(markupObject[0].tagName).get(0);
            parentElement = $(backupRangeObject.startContainer).parents(markupObject[0].tagName).get(0);
          ...
          if (backupRangeObject && backupRangeObject.endContainer.className && backupRangeObject.endContainer.className.indexOf('preparedForRemoval') > -1) {
            //var parentElement = $(backupRangeObject.endContainer).closest(markupObject[0].tagName).get(0);
            parentElement = $(backupRangeObject.endContainer).parents(markupObject[0].tagName).get(0);
            backupRangeObject.endContainer = parentElement;
            rangeObject.endContainer = parentElement;
          }
        }
        // remove all marked items
        $('.preparedForRemoval').zap();
        ...
        if (markupObject.isReplacingElement) {
          ...
          if (Ecma5Shim.Node.ELEMENT_NODE === backupRangeObject.startContainer.nodeType && backupRangeObject.startContainer.childNodes && backupRangeObject.startContainer.childNodes.length < backupRangeObject.startOffset) {
          ...
          if (Ecma5Shim.Node.ELEMENT_NODE === backupRangeObject.endContainer.nodeType && backupRangeObject.endContainer.childNodes && backupRangeObject.endContainer.childNodes.length < backupRangeObject.endOffset) {
      ...
      areMarkupObjectsAsLongAsRangeObject: function (relevantMarkupObjectsAtSelectionStart, relevantMarkupObjectsAtSelectionEnd, rangeObject) {
        ...
        for (i = 0, relMarkupStart = relevantMarkupObjectsAtSelectionStart.length; i < relMarkupStart; i++) {
          el = $(relevantMarkupObjectsAtSelectionStart[i]);
        ...
        for (i = 0, relMarkupEnd = relevantMarkupObjectsAtSelectionEnd.length; i < relMarkupEnd; i++) {
          el = $(relevantMarkupObjectsAtSelectionEnd[i]);
      ...
      splitRelevantMarkupObject: function (relevantMarkupObjectsAtSelectionStart, relevantMarkupObjectsAtSelectionEnd, rangeObject, tagComparator) {
        ...
        $(relevantMarkupObjectsAtSelectionStart).addClass('preparedForRemoval');
        $(relevantMarkupObjectsAtSelectionEnd).addClass('preparedForRemoval');
      ...
      extendExistingMarkupWithSelection: function (relevantMarkupObjects, rangeObject, startOrEnd, tagComparator) {
        ...
        for (i = 0, relMarkupLength = relevantMarkupObjects.length; i < relMarkupLength; i++) {
          ...
          if (extendMarkupsAtEnd && !extendMarkupsAtStart) {
            objects[i].startContainer = rangeObject.startContainer; // $(el).contents()[0];
            objects[i].startOffset = rangeObject.startOffset;
            textnodes = $(el).textNodes(true);
          ...
          if (!extendMarkupsAtEnd && extendMarkupsAtStart) {
            textnodes = $(el).textNodes(true);
            objects[i].startContainer = textnodes[0]; // $(el).contents()[0];
      ...
      getClonedMarkup4Wrapping: function (domobj) {
        var wrapper = $(domobj.outerHTML).removeClass('preparedForRemoval').empty();
      ...
      insertCroppedMarkups: function (relevantMarkupObjects, rangeObject, startOrEnd, tagComparator) {
        ...
        for (i = 0; i < relevantMarkupObjects.length; i++) {
          ...
          if (!cropMarkupsAtEnd && cropMarkupsAtStart) {
            objects[i].startContainer = rangeObject.endContainer; // $(el).contents()[0];
      ...
      applyMarkup: function (selectionTree, rangeObject, markupObject, tagComparator, options) {
        ...
        for (i = 0; i < optimizedSelectionTree.length; i++) {
          ...
          if (el.wrappable) {
            ...
            console.debug(this, 'dive further into non-wrappable object');
      ...
      getMarkupType: function (markupObject) {
        var nn = $(markupObject)[0].nodeName.toLowerCase();
        if (markupObject.outerHtml) {
          console.debug(this, 'Node name detected: ' + nn + ' for: ' + markupObject.outerHtml());
        }
        if (nn == '#text') {
          return 'textNode';
        }
        ...
        console.warn(this, 'unknown markup passed to this.getMarkupType(...): ' + markupObject.outerHtml());
      },
        ...
        for (i = 0; i < selectionTree.length; i++) {
          ...
          if (el.domobj && (el.selection == 'full' || (el.selection == 'partial' && markupObject.isReplacingElement))) {
            ...
            if (el.domobj.nodeType === 1 && tagComparator(el.domobj, markupObject)) {
              console.debug(this, 'Marking for removal: ' + el.domobj.nodeName);
              $(el.domobj).addClass('preparedForRemoval');
            }
          }
      ...
      wrapMarkupAroundSelectionTree: function (selectionTree, rangeObject, markupObject, tagComparator, options) {
        ...
        console.debug(this, 'The formatting <' + markupObject[0].tagName + '> will be wrapped around the selection');
        ...
        for (i = 0; i < selectionTree.length; i++) {
          ...
          if (el.domobj && !this.canTag1WrapTag2(el.domobj.parentNode.tagName.toLowerCase(), markupObject[0].tagName.toLowerCase())) {
            console.info(this, 'Skipping the wrapping of <' + markupObject[0].tagName.toLowerCase() + '> because this tag is not allowed inside <' + el.domobj.parentNode.tagName.toLowerCase() + '>');
            continue;
          }

          // skip empty text nodes
          if (el.domobj && el.domobj.nodeType === 3 && $.trim(el.domobj.nodeValue).length === 0) {
            continue;
          }
          ...
          if (el.domobj && el.selection == 'partial' && !markupObject.isReplacingElement) {
            if (el.startOffset !== undefined && el.endOffset === undefined) {
              ...
              if (el.startOffset == el.endOffset) { // do not wrap empty selections
                console.debug(this, 'skipping empty selection');
                continue;
              }
              j++;
              preText += el.domobj.data.substr(0, el.startOffset);
              ...
              console.debug(this, 'diving into object');
        ...
        if (objects2wrap.length > 0) {
          ...
          objects2wrap = $(objects2wrap);

          // make a fix for text nodes in <li>'s in ie
          $.each(objects2wrap, function (index, element) {
            if ($.browser.msie && element.nodeType == 3 && !element.nextSibling && !element.previousSibling && element.parentNode && element.parentNode.nodeName.toLowerCase() == 'li') {
      ...
      getTextNodeSibling: function (previousOrNext, commonAncestorContainer, currentTextNode) {
        var textNodes = $(commonAncestorContainer).textNodes(true), newIndex, index;
      ...
      optimizeSelectionTree4Markup: function (selectionTree, markupObject, tagComparator) {
        ...
        for (i = 0; i < selectionTree.length; i++) {
          ...
          if (selectionTree[i].domobj && selectionTree[i].selection != 'none') {
            if (markupObject.isReplacingElement && tagComparator(markupObject[0], $(selectionTree[i].domobj))) {
      ...
      canMarkupBeApplied2ElementAsWhole: function (selectionTree, markupObject) {
        ...
        for (i = 0; i < selectionTree.length; i++) {
          ...
          if (el.domobj && (el.selection != "none" || markupObject.isReplacingElement)) {
            // console.debug(this, 'Checking, if  <' + htmlTag + '> can be applied to ' + el.domobj.nodeName);
      ...
      mayInsertTag: function (tagName) {
        ...
        console.warn(this, 'Unable to determine whether tag ' + tagName + ' may be inserted');
        return true;
      },
      ...
      SelectionRange: function () {
        ...
        function Init(rangeObject) {
          var that = this;
          var r = new my.Range.RangeObject();
          // If a range object was passed in we apply the values to the new range object
          if (rangeObject) {
            if (rangeObject.commonAncestorContainer) {
              that.commonAncestorContainer = rangeObject.commonAncestorContainer;
            }
            if (rangeObject.selectionTree) {
              that.selectionTree = rangeObject.selectionTree;
            }
            if (rangeObject.limitObject) {
              that.limitObject = rangeObject.limitObject;
            }
            if (rangeObject.markupEffectiveAtStart) {
              that.markupEffectiveAtStart = rangeObject.markupEffectiveAtStart;
            }
            if (rangeObject.unmodifiableMarkupAtStart) {
              that.unmodifiableMarkupAtStart = rangeObject.unmodifiableMarkupAtStart;
            }
            if (rangeObject.splitObject) {
              that.splitObject = rangeObject.splitObject;
            }
          }
        }
         ...
         * @hide
         */
        this.limitObject = undefined;

        /**
         * DOM object being split when enter key gets hit
         * @hide
         */
        this.splitObject = undefined;
         ...
         * @hide
         */
        function update(commonAncestorContainer) {
        ...
        function getSelectionTree() {
          ...
          if (!this.selectionTree) {
            this.selectionTree = my.Selection.getSelectionTree(this);
          }
         ...
         * @hide
         */
        function getSelectedSiblings(domobj) {
         ...
         * @hide
         */
        function recursionGetSelectedSiblings(domobj, selectionTree) {
         ...
         * @hide
         */
        function updateMarkupEffectiveAtStart() {
          ...
          for (i = 0; i < parents.length; i++) {
            ...
            if (!limitFound && (el !== this.limitObject)) {
              ...
              if (!splitObjectWasSet && my.Dom.isSplitObject(el)) {
         ...
         * @hide
         */
        function updatelimitObject() {
          if (my.editables && my.editables.length > 0) {
            var parents = this.getStartContainerParents(),
              editables = my.editables,
              i,
                el,
                j,
                editable;
        ...
        return {
          Init: this.Init,
          markupEffectiveAtStart: this.markupEffectiveAtStart,
          unmodifiableMarkupAtStart: this.unmodifiableMarkupAtStart,
          limitObject: this.limitObject,
          splitObject: this.splitObject,
          select: this.select,
          update: this.update,
          getSelectionTree: this.getSelectionTree,
          getSelectedSiblings: this.getSelectedSiblings,
          recursionGetSelectedSiblings: this.recursionGetSelectedSiblings,
          updateMarkupEffectiveAtStart: this.updateMarkupEffectiveAtStart,
          updatelimitObject: this.updatelimitObject,
    ...
    function nestedListInIEWorkaround(range) {
      var nextSibling;
      if ($.browser.msie && range.startContainer === range.endContainer && range.startOffset === range.endOffset && range.startContainer.nodeType == 3 && range.startOffset == range.startContainer.data.length && range.startContainer.nextSibling) {
     ...
     * @singleton
     */
    var AlohaSelection = my.Class.extend({
