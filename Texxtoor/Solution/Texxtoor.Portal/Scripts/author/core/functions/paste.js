...
var EDITOR = (function (my) {
  ...
  //					   ' background: white; ' + // uncomment these line in debug
     ...
     *
     * @param {$.<HTMLElement>} $target Target to be cleaned
     * @return {$.<HTMLElement>}
     */
     ...
     *                             was initiated.
     * @param {$.<HTMLElement>} $target A jQuery object containing the DOM
    ...
    function rangeStartsAtProppedParagraph(range) {
      ...
      if (1 === start.nodeType) {
        return ('p' === start.nodeName.toLowerCase() &&
            ContentHandler.isProppedParagraph(start.outerHTML));
      }
     ...
     * editable.
     *
     * @param {$.<HTMLElement>} $clipboard A jQuery object containing an
    ...
    function paste($clipboard, range, callback) {
      if (range) {
        ...
        if (Command.queryCommandSupported('insertHTML')) {
          Command.execCommand('insertHTML', false, content);
        } else {
     ...
     *
     * @param {$.Event} $event Event at paste.
    ...
    function onPaste($event, range, onInsert) {
      ...
      window.setTimeout(function () {
        ...
        Core.activeEditable.smartContentChange($event);
      }, 10);
    }
     ...
     *
     * @param {$.<HTMLElement>} $editable jQuery object containing an
    ...
    var paste = {
      ...
      init: function () {
        ...
        bind('aloha-editable-created', function ($event, editable) {
