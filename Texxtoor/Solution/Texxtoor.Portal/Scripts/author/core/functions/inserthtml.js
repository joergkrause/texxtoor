...
var EDITOR = (function (my) {
  ...
  my.InsertHtml = (function () {

    // Exported commands
    my.Command.registry.register('inserthtml', {
      action: function (value, range) {
        var
          $editable = $(my.Dom.getEditingHostOf(range.startContainer)),
        ...
        function pasteElement(object) {
          ...
          if (!my.Dom.insertIntoDOM($object, range, $editable, false)) {
            ...
            if (my.Dom.isBlockLevelElement(object) || my.Dom.isListElement(object)) {
              pasteElement($('<br/>').get(0));
            }
        ...
        //}
        value = my.ContentHandlerUtils.handleContent(value, { contenthandler: Settings.contentHandler.insertHtml });
        ...
        if (domNodes.length > 0) {
          range = my.Dom.setCursorAfter(domNodes.get(domNodes.length - 1));
        } else {
          // if nothing was pasted, just reselect the old range
          range.select();
        }

        my.Dom.doCleanup({ merge: true, removeempty: true }, range, cac);
