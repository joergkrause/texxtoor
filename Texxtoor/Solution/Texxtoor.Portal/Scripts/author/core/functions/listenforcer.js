/* listenforcer.js is part of Aloha Editor project http://aloha-editor.org
 *
...
var EDITOR = (function (my) {
  ...
  my.ListEnforcer = (function () {
    ...
    function enforce($editable, placeHolderListString) {
      ...
      if ($.inArray($editable[0], listEnforcedEditables) === -1) {
        return;
      }
      ...
      if (j > 1) {
        ...
        for (i = 1; i < j; ++i) {
          $firstList.append($($lists[i]).find('>li'));
          $($lists[i]).remove();
        }
      }
    ...
    return {
      ...
      init: function () {
        ...
        bind('aloha-editable-activated', function ($event, params) {
          enforce(params.editable.obj,
            '<ul><li><br /></li></ul>');
        });

        bind('aloha-editable-deactivated', function ($event, params) {
          enforce(params.editable.obj, '');
        });

        bind('aloha-smart-content-changed', function ($event, params) {
          //We only want to do this is if the editable is actually active
          if (my.Core.activeEditable && my.Core.activeEditable.isActive === true) {
            enforce(params.editable.obj,
              '<ul><li><br /></li></ul>');
          }
        });
