...
var EDITOR = (function (my) {
  ...
  my.Command = (function () {
    ...
    // CTOR
    function commandManager(registry) {
    ...
    commandManager.prototype = {
      ...
      execCommand: function (commandId, showUi, value, range) {
        ...
        trigger('aloha-command-will-execute', evtObj);
        ...
        if (!range) {
          if (!my.Selection.getSelection().getRangeCount()) {
            return;
          }
          range = my.Selection.getSelection().getRangeAt(0);
        }
        ...
        if (commandId.toLowerCase() === 'inserthtml') {
          ...
          //}
          value = my.ContentHandlerUtils.handleContent(value, {
            contenthandler: my.Settings.contentHandler.insertHtml,
            command: 'insertHtml'
          });
        }

        my.Engine.execCommand(commandId, showUi, value, range);
        ...
        trigger('aloha-command-executed', commandId);
      },
      ...
      queryCommandEnabled: function (commandId, range) {
        ...
        return my.Engine.queryCommandEnabled(commandId, range);
      },
      ...
      queryCommandIndeterm: function (commandId, range) {
        ...
        return my.Engine.queryCommandIndeterm(commandId, range);
      ...
      queryCommandState: function (commandId, range) {
        ...
        return my.Engine.queryCommandState(commandId, range);
      ...
      queryCommandSupported: function (commandId) {
        ...
        return my.Engine.queryCommandSupported(commandId);
      },
      ...
      queryCommandValue: function (commandId, range) {
        ...
        return my.Engine.queryCommandValue(commandId, range);
      },
      querySupportedCommands: function () {
        ...
        for (command in my.Engine.commands) {
          if (my.Engine.commands.hasOwnProperty(command)) {
            commands.push(command);
          }
        }
        return commands;
      },
      getStateOverride: my.Engine.getStateOverride,
      setStateOverride: my.Engine.setStateOverride,
      resetOverrides: my.Engine.resetOverrides,
      unsetStateOverride: my.Engine.unsetStateOverride
    };
     ...
     */
    my.Engine.execCommand = commandManager.execCommand;
     ...
     */
    my.Engine.queryCommandEnabled = commandManager.queryCommandEnabled;
     ...
     * @return true if command is indeterminate, otherwise false.
     */
    my.Engine.queryCommandIndeterm = commandManager.queryCommandIndeterm;
     ...
     */
    my.Engine.queryCommandState = commandManager.queryCommandState;

    /**
     * Check if a given command is supported
     * @return true if command is supported, and false otherwise.
     */
    my.Engine.queryCommandSupported = commandManager.queryCommandSupported;
     ...
     * @return command's value.
     */
    my.Engine.queryCommandValue = commandManager.queryCommandValue;

    my.Engine.querySupportedCommands = commandManager.querySupportedCommands;

    // create an instance
    var commandmanager = new commandManager(my.Registry);
