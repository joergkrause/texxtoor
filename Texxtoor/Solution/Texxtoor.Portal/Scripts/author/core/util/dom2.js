  ...
  my.Dom2 = (function () {
    ...
    function indexByClassHaveList(elems, classMap) {
      ...
      for (i = 0, len = elems.length; i < len; i++) {
        elem = elems[i];
        if (elem.className) {
          classes = my.Strings.words(elem.className);
    ...
    function walk(node, fn, arg) {
      walkUntil(node, fn, Functions.returnFalse, arg);
    }
    ...
    StableRange.prototype.update = function () {
      ...
      this.commonAncestorContainer = my.Arrays.intersect(start, end)[0];
    };
    ...
    function extendToWord(range) {
      var rangeObject = new range.RangeObject(range);
    ...
    function getStyle(node, name) {
      ...
      name = my.Strings.dashesToCamelCase(name);
