  ...
  my.Dom = (function () {
    ...
    function findNodeForward(node, match) {
      ...
      var next = node.firstChild
              ...
              || (
                  node.parentNode
                  && !Editor.Dom.isEditingHost(node.parentNode)
    ...
    */
     ...
     */
      split: function (range, limit, atEnd) {
        var splitElement = $(range.startContainer),
        ...
        if (atEnd) {
          splitElement = $(range.endContainer);
        ...
        for (i = 0, pathLength = path.length; i < pathLength; ++i) {
          ...
          if (i === pathLength - 1) {
            ...
            if (element.nodeType === 3) {
              ...
              if (this.isEmpty(secondPart) && $('br', newDom).length === 0) {
                secondPart = $('<br/>').addClass('aloha-end-br');
              }
            } else {
              // other nodes
              jqelement = $(element);
            ...
            newElement = $(element).clone(false).empty();
        ...
        return $([path[0], newDom ? newDom.get(0) : secondPart]);
      },
     ...
     */
      allowsNesting: function (outerDOMObject, innerDOMObject) {
        ...
        if ($.isArray(this.children[outerNodeName]) && $.inArray(innerNodeName, this.children[outerNodeName]) >= 0) {
          return true;
        }

        if ($.isArray(this.tags[this.children[outerNodeName]])
          && $.inArray(innerNodeName, this.tags[this.children[outerNodeName]]) >= 0) {
          return true;
        }
     ...
     */
      addMarkup: function (rangeObject, markup, nesting) {
        ...
        if (rangeObject.startContainer.nodeType === 3
          ...
          this.split(rangeObject, $(rangeObject.startContainer).parent(), false);
        }
        if (rangeObject.endContainer.nodeType === 3 && rangeObject.endOffset > 0 && rangeObject.endOffset < rangeObject.endContainer.data.length) {
          this.split(rangeObject, $(rangeObject.endContainer).parent(), true);
        }
      ...
      recursiveAddMarkup: function (rangeTree, markup, rangeObject, nesting) {
        ...
        for (i = 0, rangeLength = rangeTree.length; i < rangeLength; ++i) {
          ...
          if (rangeTree[i].type == 'full' && this.allowsNesting(markup.get(0), rangeTree[i].domobj)) {
            ...
            if ((nesting || rangeTree[i].domobj.nodeName != markup.get(0).nodeName) && (rangeTree[i].domobj.nodeType !== 3 || $.trim(rangeTree[i].domobj.data).length !== 0)) {
              // wrap the object
              $(rangeTree[i].domobj).wrap(markup);
              ...
              if (!nesting && rangeTree[i].domobj.nodeType !== 3) {
                innerRange = new my.Range.RangeObject();
                ...
                this.removeMarkup(innerRange, markup, $(rangeTree[i].domobj.parentNode));
              }
            }
          } else {
     ...
     */
      removeMarkup: function (rangeObject, markup, limit) {
        ...
        if (startSplitLimit && rangeObject.startOffset > 0) {
          ...
          this.split(rangeObject, $(startSplitLimit).parent(), false);
          didSplit = true;
        }
        ...
        if (endSplitLimit) {
          ...
          if (rangeObject.endContainer.nodeType === 3 && rangeObject.endOffset < rangeObject.endContainer.data.length) {
            this.split(rangeObject, $(endSplitLimit).parent(), true);
            didSplit = true;
          }
            ...
            case 1:
              if (prevNode && prevNode.nodeName == this.nodeName) {
                ...
                if (cleanup.removeempty) {
                  if (Dom.isBlockLevelElement(this) && this.childNodes.length === 0) {
                    //							$(this).remove();
                    removed = true;
                  }
                  if ($.inArray(this.nodeName.toLowerCase(), that.mergeableTags) >= 0 && $(this).text().length === 0 && this.childNodes.length === 0) {
                    //							$(this).remove();
                    removed = true;
                  }
                }
                ...
                if (!removed) {
                  if (cleanup.mergeable
                    ...
                    : $.inArray(this.nodeName.toLowerCase(), that.mergeableTags) >= 0) {
                    prevNode = this;
                  } else {
                    prevNode = false;
                  }
     ...
     */
      isBlockLevelElement: function (node) {
        ...
        if (node.nodeType === 1 && $.inArray(node.nodeName.toLowerCase(), this.blockLevelElements) >= 0) {
          return true;
        }
        return false;
      },
     ...
     */
      isListElement: function (node) {
        ...
        return node.nodeType === 1 && $.inArray(node.nodeName.toLowerCase(), this.listElements) >= 0;
      },
      ...
      searchAdjacentTextNode: function (parent, index, searchleft, stopat) {
        ...
        while (currentParent) {
          ...
          if (!nextNode) {
            ...
            continue;
          } else if (nextNode.nodeType === 3 && $.trim(nextNode.data).length > 0) {
     ...
     */
      insertIntoDOM: function (object, range, limit, atEnd, force) {
        ...
        if (parentElements.length === 0) {
          ...
          $.each(parentElements, function (index, parent) {
        ...
        if (typeof newParent !== 'undefined') {
          ...
          splitParts = this.split(range, $(newParent), atEnd);
          if (splitParts === true) {
            ...
            $(container).contents().eq(offset - 1).after(object);
            return true;
          }
          if (splitParts) {
      ...
      removeFromDOM: function (object, range, preserveContent) {
        if (preserveContent) {
          ...
          var indexInParent = this.getIndexInParent(object),
              numChildren = $(object).contents().length,
      ...
      recursiveRemoveRange: function (rangeTree, rangeObject) {
        ...
        for (i = 0; i < rangeTree.length; ++i) {
          ...
          if (rangeTree[i].type == 'full') {
            ...
            if ($(rangeObject.startContainer).parents().andSelf().filter(rangeTree[i].domobj).length > 0) {
     ...
     */
      isWordBoundaryElement: function (object) {
        ...
        return $.inArray(object.nodeName.toLowerCase(), this.nonWordBoundaryTags) == -1;
      },
     ...
     */
      isEmpty: function (domObject) {
        ...
        if ($.inArray(domObject.nodeName.toLowerCase(), this.nonEmptyTags) != -1) {
          return false;
        }
     ...
     */
      setCursorAfter: function (domObject) {
        var newRange = new my.Range.RangeObject(),
     ...
     */
      setCursorInto: function (domObject) {
        ...
        var newRange = new Range.RangeObject();
     ...
     */
      getFirstVisibleChild: function (node, includeNode) {
        ...
        if ((node.nodeType == $_.Node.TEXT_NODE && this.isEmpty(node)) || (node.nodeType == $_.Node.ELEMENT_NODE && node.offsetHeight == 0 && $.inArray(node.nodeName.toLowerCase(), this.nonEmptyTags) === -1)) {
          return null;
        }

        // if the node is a text node, or does not have children, or is not editable, it is the first visible child
        if (node.nodeType == $_.Node.TEXT_NODE || (node.nodeType == $_.Node.ELEMENT_NODE && node.childNodes.length == 0) || !$(node).contentEditable()) {
