...
var EDITOR = (function (my) {
  ...
  my.WordContentHandler = (function () {
  ...
  function transformTitles($content) {
    $content.find('p.MsoTitle').each(function () {
      Editor.Markup.transformDomObject($(this), 'h1');
    });
    $content.find('p.MsoSubtitle').each(function () {
      Editor.Markup.transformDomObject($(this), 'h2');
    });
  }

  var wordContentHandler = {
    ...
    transformListsFromWord: function (content) {
      ...
      if (paragraphs.length > 0) {
        paragraphs.each(function () {
          ...
          firstSpan = $(jqElem.find('span.' + bulletClass));
    ...
    removeParagraphNumbering: function (content) {
      ...
      if (paragraphs.length > 0) {
        paragraphs.each(function () {
          ...
          spans.each(function() {
            if ($.trim($(this).text()).match(/^([\.\(]?[\d\D][\.\(]?){1,4}$/)) {
              $(this).remove();
            }
          });
    ...
    */
    transformToc: function (content) {
      ...
      paragraphs.each(function () {
        ...
        spans.each(function () {
          if ($(this).attr('style') && $(this).attr('style').search('mso-hide') > -1) {
            $(this).remove();
          }
          $(this).contents().unwrap();
        });
