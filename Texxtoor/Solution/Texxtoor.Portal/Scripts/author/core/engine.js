...
var EDITOR = (function (my) {
  ...
  my.Engine = function () {
    ...
    function getLegacyFontSize(size) {
      ...
      if ($.inArray(size, ["xx-small", "x-small", "small", "medium", "large", "x-large", "xx-large", "xxx-large"]) == -1 && !/^[0-9]+(\.[0-9]+)?(cm|mm|in|pt|pc|px)$/.test(size)) {
    ...
    function myQueryCommandEnabled(command, range) {
      ...
      return editCommandMethod(command, "action", range, (function (command) {
        return function () {
          ...
          // otherwise."
          return $.inArray(command, ["copy", "cut", "paste", "selectall", "stylewithcss", "usecss"]) != -1 || range !== null;
        };
      }(command)));
    }

    function setActiveRange(range) {
      var rangeObject = new my.Range.RangeObject();
    ...
    function isEditable(node) {
      ...
      return node && !isEditingHost(node) && (node.nodeType != $_.Node.ELEMENT_NODE || node.contentEditable != "false" || $(node).hasClass('aloha-table-wrapper')) && (isEditingHost(node.parentNode) || isEditable(node.parentNode));
    }
    ...
    function isWhitespaceNode(node) {
      return node && node.nodeType == $_.Node.TEXT_NODE && (node.data == "" || (/^[\t\n\r ]+$/.test(node.data) && node.parentNode && node.parentNode.nodeType == $_.Node.ELEMENT_NODE && $.inArray($_.getComputedStyle(node.parentNode).whiteSpace, ["normal", "nowrap"]) != -1) || (/^[\t\r ]+$/.test(node.data) && node.parentNode && node.parentNode.nodeType == $_.Node.ELEMENT_NODE && $_.getComputedStyle(node.parentNode).whiteSpace == "pre-line") || (/^[\t\n\r ]+$/.test(node.data) && node.parentNode && node.parentNode.nodeType == $_.Node.DOCUMENT_FRAGMENT_NODE));
    }
    ...
    function collapseWhitespace(node, range) {
      ...
      if ($.inArray($_.getComputedStyle(node.parentNode).whiteSpace, ["pre", "pre-wrap"]) != -1) {
        return;
      }
    ...
    function getActiveRange() {
      ...
      if (globalRange) {
        ret = globalRange;
      } else if (my.getSelection().rangeCount) {
        ret = my.getSelection().getRangeAt(0);
      } else {
        return null;
      }
      if ($.inArray(ret.startContainer.nodeType, [$_.Node.TEXT_NODE, $_.Node.ELEMENT_NODE]) == -1 || $.inArray(ret.endContainer.nodeType, [$_.Node.TEXT_NODE, $_.Node.ELEMENT_NODE]) == -1 || !ret.startContainer.ownerDocument || !ret.endContainer.ownerDocument || !isDescendant(ret.startContainer, ret.startContainer.ownerDocument) || !isDescendant(ret.endContainer, ret.endContainer.ownerDocument)) {
    ...
    function movePreservingRanges(node, newParent, newIndex, range) {
      ...
      // impossible).
      var ranges = [range];
      for (i = 0; i < my.getSelection().rangeCount; i++) {
        ranges.push(my.getSelection().getRangeAt(i));
      }
      var boundaryPoints = [];
    ...
    function copyAttributes(element, newElement) {
      ...
      if ($.browser.msie && $.browser.version >= 7 && typeof element.attributes[$.expando] !== 'undefined') {
        $(element).removeAttr($.expando);
      }
    ...
    var prohibitedHeadingNestingMap = $.extend({
      "H1": true,
      "H2": true,
      "H3": true,
      "H4": true,
    ...
    function isAllowedChild(child, parent_) {
      ...
      switch (parent_) {
        case "colgroup":
          return child == "col";
        case "table":
          return $.inArray(child, ["caption", "col", "colgroup", "tbody", "td", "tfoot", "th", "thead", "tr"]) != -1;
        case "tbody":
        case "thead":
        case "tfoot":
          return $.inArray(child, ["td", "th", "tr"]) != -1;
        case "tr":
          return $.inArray(child, ["td", "th"]) != -1;
        case "dl":
          return $.inArray(child, ["dt", "dd"]) != -1;
        case "dir":
        case "ol":
        case "ul":
          return $.inArray(child, ["dir", "li", "ol", "ul"]) != -1;
        case "hgroup":
      ...
      // false."
      if ($.inArray(child, ["body", "caption", "col", "colgroup", "frame", "frameset", "head", "html", "tbody", "td", "tfoot", "th", "thead", "tr"]) != -1) {
        return false;
      }

      // "If child is "dd" or "dt" and parent is not "dl", return false."
      if ($.inArray(child, ["dd", "dt"]) != -1 && parent_ != "dl") {
        return false;
      }
    ...
    function isModifiableElement(node) {
      ...
      if ($.inArray(node.tagName, ["B", "EM", "I", "S", "SPAN", "STRIKE", "STRONG", "SUB", "SUP", "U"]) != -1) {
    ...
    function isSimpleModifiableElement(node) {
      ...
      if ($.inArray(node.tagName, ["A", "B", "EM", "FONT", "I", "S", "SPAN", "STRIKE", "STRONG", "SUB", "SUP", "U"]) == -1) {
        return false;
      }
      ...
      // "overline" or "none"."
      if ($.inArray(node.tagName, ["A", "FONT", "S", "SPAN", "STRIKE", "U"]) != -1 && hasAttribute(node, "style") && getStyleLength(node) == 1 && (node.style.textDecoration == "line-through" || node.style.textDecoration == "underline" || node.style.textDecoration == "overline" || node.style.textDecoration == "none")) {
        return true;
      }
    ...
    function forceValue(node, command, newValue, range) {
      ...
      if (!cssStylingFlag) {
        ...
        if (command == "forecolor" && parseSimpleColor(newValue)) {
          ...
          $(newParent).css('color', parseSimpleColor(newValue));
        }
      ...
      if (command == "fontsize" && $.inArray(newValue, ["xx-small", "small", "medium", "large", "x-large", "xx-large", "xxx-large"]) != -1 && (!cssStylingFlag || newValue == "xxx-large")) {
        newParent = node.ownerDocument.createElement("font");
    ...
    function setSelectionValue(command, newValue, range) {
      ...
      if (isEditable(range.startContainer) && range.startContainer.nodeType == $_.Node.TEXT_NODE && range.startOffset != 0 && range.startOffset != getNodeLength(range.startContainer)) {
        ...
        var newActiveRange = my.createRange();
        ...
        my.getSelection().removeAllRanges();
        my.getSelection().addRange(newActiveRange);
      ...
      if (isEditable(range.endContainer) && range.endContainer.nodeType == $_.Node.TEXT_NODE && range.endOffset != 0 && range.endOffset != getNodeLength(range.endContainer)) {
        ...
        my.getSelection().removeAllRanges();
        my.getSelection().addRange(activeRange);
      }
    ...
    function splitParent(nodeList, range) {
      ...
      if ($.inArray(originalParent.firstChild, nodeList) != -1) {
        removeExtraneousLineBreaksBefore(originalParent);
      }

      var firstChildInNodeList = $.inArray(originalParent.firstChild, nodeList) != -1;
      var lastChildInNodeList = $.inArray(originalParent.lastChild, nodeList) != -1;
    ...
    commands.fontsize = {
      action: function (value, range) {
        ...
        if ($.inArray(value, ["xx-small", "x-small", "small", "medium", "large", "x-large", "xx-large", "xxx-large"]) == -1 && !/^[0-9]+(\.[0-9]+)?(cm|mm|in|pt|pc)$/.test(value)) {
          return;
        }
    ...
    function isSimpleIndentationElement(node) {
      ...
      for (i = 0; i < node.attributes.length; i++) {
        if (!isHtmlNamespace(node.attributes[i].namespaceURI) || $.inArray(node.attributes[i].name, ["style", "class", "dir"]) == -1) {
          return false;
        }
      }
    ...
    function ensureContainerEditable(container) {
      ...
      } else if ($.browser.msie && $.browser.version <= 7 && isHtmlElementInArray(container, ["p", "h1", "h2", "h3", "h4", "h5", "h6", "pre", "blockquote"])) {
      ...
      while (!isAllowedChild(node, node.parentNode)) {
        ...
        if (node.parentNode.childNodes.length == 1) {
          ...
          if (range.startContainer === node.parentNode && range.startOffset > getNodeIndex(node)) {
            ...
            newStartOffset = range.startOffset + ($(node).contents().length - 1);
          }
          if (range.endContainer === node.parentNode && range.endOffset > getNodeIndex(node)) {
            // the node (1 element) will be replaced by its contents (contents().length elements)
            newEndOffset = range.endOffset + ($(node).contents().length - 1);
          }
  ...
  return (isNamedHtmlElement(node, 'ul')
    ...
    function getAlignmentValue(node) {
      ...
      // its parent."
      while ((node && node.nodeType != $_.Node.ELEMENT_NODE) || (node.nodeType == $_.Node.ELEMENT_NODE && $.inArray($_.getComputedStyle(node).display, ["inline", "none"]) != -1)) {
        node = node.parentNode;
      }
      ...
      if ($.inArray(resolvedValue, ["center", "justify", "left", "right"]) != -1) {
    ...
    function canonicalizeWhitespace(node, offset) {
      ...
      while (true) {
        ...
        } else if (startNode.nodeType == $_.Node.TEXT_NODE && $.inArray($_.getComputedStyle(startNode.parentNode).whiteSpace, ["pre", "pre-wrap"]) == -1 && startOffset != 0 && /[ \xa0]/.test(startNode.data[startOffset - 1])) {
          startOffset--;
      ...
      while (true) {
        ...
        } else if (endNode.nodeType == $_.Node.TEXT_NODE && $.inArray($_.getComputedStyle(endNode.parentNode).whiteSpace, ["pre", "pre-wrap"]) == -1 && endOffset != getNodeLength(endNode) && /[ \xa0]/.test(endNode.data[endOffset])) {
    ...
    function cleanLists(node, range) {
      ...
      if (node) {
        $(node).find('ul,ol,li').each(function () {
    ...
    commands["delete"] = {
      action: function (value, range) {
        ...
        if (node.nodeType == $_.Node.TEXT_NODE && offset == 0 && $.browser.msie) {
          offset = 1;
          range.setStart(node, offset);
          range.setEnd(node, offset);
          range.startOffset = 0;
    ...
    commands.formatblock = {
      action: function (value) {
        ...
        while (nodeList.length) {
          ...
          fixDisallowedAncestors(wrap(
            sublist,
            $.inArray(value, ["div", "p"]) == -1 ? makeIsElementWithoutAttributes(value) : returnFalse,
    ...
    commands.inserthorizontalrule = {
      action: function (value, range) {
        ...
        my.getSelection().removeAllRanges();
        my.getSelection().addRange(range);
      }
    };
    ...
    commands.insertlinebreak = {
      action: function (value, range) {
        ...
        if (range.startContainer.nodeType == $_.Node.TEXT_NODE && range.startOffset == 0) {
          ...
          my.getSelection().collapse(newNode, newOffset);
        ...
        // argument."
        my.getSelection().collapse(br.parentNode, 1 + getNodeIndex(br));
    ...
    commands.insertparagraph = {
      action: function (value, range) {
        ...
        my.getSelection().collapse(node, offset);
        range.setStart(node, offset);
        range.setEnd(node, offset);
        ...
        if (!isEditable(container) || !inSameEditingHost(container, node) || !isSingleLineContainer(container)) {
          ...
          if (!nodeList.length) {
            ...
            my.getSelection().collapse(container, 0);
        ...
        if (container.tagName == "ADDRESS" || container.tagName == "LISTING" || container.tagName == "PRE") {
          ...
          // Selection."
          my.getSelection().collapse(node, offset + 1);
          range.setStart(node, offset + 1);
          range.setEnd(node, offset + 1);
        ...
        // container)."
        var newLineRange = my.createRange();
        ...
        my.getSelection().collapse(newContainer, 0);
    ...
    commands.inserttext = {
      action: function (value, range) {
        ...
        if (value == " " && refElement.nodeType == $_.Node.ELEMENT_NODE && $.inArray($_.getComputedStyle(refElement).whiteSpace, ["pre", "pre-wrap"]) == -1) {
          value = "\xa0";
        }
        ...
        if (node.nodeType == $_.Node.TEXT_NODE) {
          ...
          // Selection."
          my.getSelection().extend(node, offset + 1);
          range.setEnd(node, offset + 1);
          ...
          my.getSelection().collapse(text, 0);
          range.setStart(text, 0);

          // "Call extend(text, 1) on the context object's Selection."
          my.getSelection().extend(text, 1);
          range.setEnd(text, 1);
        }
    ...
    commands.selectall = {
      ...
      action: function () {
        ...
        if (!target) {
          ...
          my.getSelection().selectAllChildren(target);
        }
      }
    };
