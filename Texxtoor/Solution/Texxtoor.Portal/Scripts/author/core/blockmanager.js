...
var EDITOR = (function (my) {
   ...
   */
    function blockManager() {
      my.Scopes.createScope('Aloha.Block');
      this.blockTypes = my.Registry;
      this.blocks = my.Registry;
    ...
    blockManager.prototype = {
      ...
      registerEventHandlers: function () {
        ...
        my.bind('aloha-selection-changed', function (evt, selection, originalEvent) {
      ...
      _registerEventHandlersForBlockDeletion: function () {
        ...
        bind('aloha-command-will-execute', function (e, data) {
          ...
          var onlyBlockSelected = (my.Selection.getSelection().getRangeCount() === 0) || // Firefox / Chrome
            (my.Selection.getSelection().getRangeCount() === 1 && my.Selection.getSelection().getRangeAt(0).endContainer === Selection.getSelection().getRangeAt(0).startContainer && my.Selection.getSelection().getRangeAt(0).endContainer === $('body')[0]) || // Internet explorer: Inline Elements
            (my.Selection.getSelection().getRangeCount() === 1 && my.Selection.getSelection().getRangeAt(0).endContainer === Selection.getSelection().getRangeAt(0).startContainer && my.Selection.getSelection().getRangeAt(0).startOffset + 1 === my.Selection.getSelection().getRangeAt(0).endOffset); // Internet explorer: Block level elements
          ...
          if (that._activeBlock && (commandId === 'delete' || commandId === 'forwarddelete') && onlyBlockSelected) {
          ...
          } else if (!that._activeBlock && (commandId === 'delete' || commandId === 'forwarddelete') && Selection.getSelection().getRangeCount() === 1 && Selection.getSelection().getRangeAt(0).collapsed === false) {
            ...
            traverseSelectionTree(my.Selection.getSelectionTree());
          }
        });
        ...
        $(window.document).keydown(function (e) {
          ...
          if (that._activeBlock && (e.which === 46 || e.which === 8) && that._activeBlock._isInsideNestedEditable === false) {
            ...
            if ($.browser.msie && parseInt($.browser.version, 10) <= 8 && that._activeBlock.$element.parents('.aloha-editable,.aloha-block').first().hasClass('aloha-editable')) {
              that._activeBlock.destroy();
              e.preventDefault();
              return false;
      ...
      _registerEventHandlersForCutCopyPaste: function () {
        ...
        $(window.document).keydown(function (e) {
          if (my.Core.activeEditable && that._activeBlock) {
            ...
            // in the editable
            if ($.contains(that._activeBlock.$element.get(0), my.Core.activeEditable.obj.get(0))) {
              return;
            }
          }
          ...
          if (that._activeBlock && (e.ctrlKey || e.metaKey) && e.which === 67) {
            ...
            my.Dom.selectDomNode(that._activeBlock.$element[0]);
          }
      ...
      _registerEventHandlersForEditableActivated: function () {
        var that = this;
        my.bind('aloha-editable-activated', function (event, arg) {
          if (arg.editable) {
      ...
      initializeBlockLevelDragDrop: function () {
        var that = this;
        $.each(Core.editables, function (i, editable) {
        ...
        my.bind('aloha-editable-created', function (e, editable) {
          editable.obj.data("block-dragdrop", that._dragdropEnabled);
          that.createBlockLevelSortableForEditableOrBlockCollection(editable.obj);
        });
      },
      ...
      registerBlockType: function (identifier, blockType) {
        my.Scopes.createScope('Aloha.Block.' + identifier, 'Aloha.Block');
      ...
      _blockify: function (element, instanceDefaults) {
        var that = this,
          $element = $(element),
          BlockPlugin = my.require('block/block-plugin'),
          tagName = $element[0].tagName.toLowerCase(),
          attributes,
          block;

        if ($.inArray(tagName, BlockPlugin.settings.rootTags) === -1) {
          console.error('block/blockmanager', 'Blocks can only be created from [' +
        ...
        if (!this.blockTypes.has(attributes['aloha-block-type'])) {
          console.error('block/blockmanager', 'Block Type ' + attributes['aloha-block-type'] + ' not found!');
          return;
        }
      ...
      getConfig: function (blockElement, instanceDefaults) {
        ...
        // We use $.clone(true) because the sortableItem attribute isn't returned
        // if we do a normal cloneNode(...).
        var clone = blockElement.clone(true);
        var dataCamelCase = clone.data();
        var data = {};
        clone.removeData();
        // $.data() returns data attributes with names like
      ...
      _deactivateHighlightedBlocks: function () {
        $.each($.extend({}, this._highlightedBlocks), function (id) {
      ...
      _getHighlightedBlocks: function () {
        ...
        $.each(this.blocks.getEntries(), function (blockId, block) {
          if (block.isActive()) {
            highlightedBlocks[blockId] = block;
          }
        });
