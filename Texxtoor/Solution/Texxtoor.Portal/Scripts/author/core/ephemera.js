...
var EDITOR = (function (my) {
  ...
  my.Ephemera = (function () {
     ...
     */
    function classes() {
      var clss = my.Array.prototype.slice.call(arguments);
      my.Maps.fillKeys(ephemeraMap.classMap, clss, true);
      checkCommonSubstr(clss);
      my.PubSub().pub('aloha.ephemera.classes', {
     ...
     */
    function attributes() {
      var attrs = my.Array.prototype.slice.call(arguments);
      my.Maps.fillKeys(ephemeraMap.attrMap, attrs, true);
      my.PubSub().pub('aloha.ephemera.attributes', {
        ephemera: ephemeraMap,
        newAttributes: attrs
      });
    }
    ...
    function markAttr(elem, attr) {
      ...
      if (null == data || '' === data) {
        data = attr;
      } else if (-1 === my.Arrays.indexOf(Strings.words(data), attr)) {
        data += ' ' + attr;
      }
    ...
    function isAttrEphemeral(elem, attrName, attrMap, attrRxs) {
      ...
      if (mapped) {
        ...
        if (-1 !== my.Arrays.indexOf(mapped, elem.nodeName)) {
          return true;
        }
      }
      return my.Misc.anyRx(attrRxs, attrName);
    }
    ...
    function pruneElem(elem, emap) {
      ...
      if (className && -1 !== className.indexOf(commonClsSubstr)) {
        var classes = my.Strings.words(className);

        // Ephemera.markElement()
        if (-1 !== my.Arrays.indexOf(classes, 'aloha-cleanme') || -1 !== my.Arrays.indexOf(classes, 'aloha-ephemera')) {
          $.removeData(elem); // avoids memory leak
          return false; // removes the element
        }

        // Ephemera.markWrapper() and Ephemera.markFiller()
        if (-1 !== my.Arrays.indexOf(classes, 'aloha-ephemera-wrapper') || -1 !== my.Arrays.indexOf(classes, 'aloha-ephemera-filler')) {
          my.Dom.moveNextAll(elem.parentNode, elem.firstChild, elem.nextSibling);
          $.removeData(elem);
          return false;
        }

        // Ephemera.markAttr()
        if (-1 !== my.Arrays.indexOf(classes, 'aloha-ephemera-attr')) {
          pruneMarkedAttrs(elem);
        }

        // Ephemera.classes() and Ehpemera.ephemera({ classMap: {} })
        var persistentClasses = my.Arrays.filter(classes, function (cls) {
    ...
    function pruneStep(emap, step, node) {
      if (1 === node.nodeType) {
        ...
        node = my.Trees.walkDomInplace(node, step);
      }

      // Ephemera.ephemera({ pruneFns: [] })
      node = my.Arrays.reduce(emap.pruneFns, node, my.Arrays.applyNotNull);
      if (!node) {
        return [];
      }
