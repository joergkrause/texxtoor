...
var EDITOR = (function (my) {
  ...
  my.Markup = (function () {
    ...
    var isOldIE = !!($.browser.msie && 9 > parseInt($.browser.version, 10));
    ...
    function isBlock(node) {
      return 'false' === $(node).attr('contenteditable');
    }
    ...
    function nodeContains(node1, node2) {
      return isOldIE ? (Ecma5Shim.compareDocumentPosition(node1, node2) & 16) : 0 < $(node1).find(node2).length;
    }
    ...
    function jumpBlock(block, isGoingLeft, currentRange) {
      var range = new my.Range.RangeObject();
      ...
      my.trigger('aloha-block-selected', block);
      my.Selection.preventSelectionChanged();
    }
    ...
    return {
      ...
      insertBreak: function() {
        ...
        newBreak = $('<br/>');
        my.Dom.insertIntoDOM(newBreak, range, my.Core.activeEditable.obj);

        nextTextNode = my.Dom.searchAdjacentTextNode(
          newBreak.parent().get(0),
          my.Dom.getIndexInParent(newBreak.get(0)) + 1,
          false
        );
        ...
        range.startOffset = range.endOffset = my.Dom.getIndexInParent(newBreak.get(0)) + 1;
        range.correctRange();
        range.clearCaches();
        range.select();
      },
      ...
      preProcessKeyStrokes: function(event) {
        ...
        if (event.keyCode === 37 || event.keyCode === 39) {
          if (my.getSelection().getRangeCount()) {
            rangeObject = my.getSelection().getRangeAt(0);

            if (this.processCursor(rangeObject, event.keyCode)) {
              cleanupPlaceholders(my.Selection.rangeObject);
              return true;
            }
          }
        ...
        if (event.keyCode === 8) {
          ...
          my.execCommand('delete', false);
          return false;
        }

        // DELETE
        if (event.keyCode === 46) {
          my.execCommand('forwarddelete', false);
          return false;
        }

        // ENTER
        if (event.keyCode === 13) {
          if (!event.shiftKey && Html.allowNestedParagraph(my.activeEditable)) {
            my.execCommand('insertparagraph', false);
            return false;
            ...
            my.execCommand('insertlinebreak', false);
            return false;
          }
        }
        return true;
      ...
      processCursor: function(range, keyCode) {
        ...
        if ($.browser.msie && parseInt($.browser.version, 10) <= 7 && isTextNode(node)) {
          if (keyCode == 37) {
        ...
        if (isOldIE && !$(node).contentEditable()) {
          var $parentBlock = $(node).parents('[contenteditable=false]');
      ...
      processEnter: function(rangeObject) {
        if (rangeObject.splitObject) {
          ...
          //  $( rangeObject.splitObject ).append(
      ...
      insertHTMLCode: function(html) {
        ...
        this.insertHTMLBreak(rangeObject.getSelectionTree(), rangeObject, $(html));
      },
      ...
      insertHTMLBreak: function(selectionTree, rangeObject, inBetweenMarkup) {
        ...
        inBetweenMarkup = inBetweenMarkup || $('<br/>');

        for (i = 0, treeLength = selectionTree.length; i < treeLength; ++i) {
          el = selectionTree[i];
          jqEl = el.domobj ? $(el.domobj) : undefined;
          ...
          if (el.selection !== 'none') { // before cursor, leave this part inside the splitObject
            if (el.selection == 'collapsed') {
              ...
              if (i > 0) {
                ...
                jqElBefore = $(selectionTree[i - 1].domobj);
                ...
                jqElAfter = $(selectionTree[1].domobj);
              ...
              rangeObject.startOffset = rangeObject.endOffset = my.Dom.getIndexInParent(inBetweenMarkup[0]) + 1;
              ...
              if (el.domobj.nextSibling && el.domobj.nextSibling.nodeType == 1 && Selection.replacingElements[el.domobj.nextSibling.nodeName.toLowerCase()]) {
              ...
              if (this.needEndingBreak()) {
                ...
                while (checkObj) {
                  if (checkObj.nextSibling) {
                    ...
                    if (my.Dom.isBlockLevelElement(checkObj) || my.Dom.isListElement(checkObj)) {
                      break;
                    }
                ...
                // content is fetched.
                if (checkObj) {
                  $(checkObj).append('<br class="aloha-cleanme" />');
                }
              }
              ...
              if (jqEl.parent().find('br.aloha-ephemera').length === 0) {
                ...
                $(rangeObject.limitObject).find('br.aloha-ephemera').remove();

                //  now put it:
                $(rangeObject.commonAncestorContainer).append(this.getFillUpElement(rangeObject.splitObject));
              }
          ...
          if (el.selection == 'partial') {
            if (el.domobj.nodeType === 3) {
              ...
              if (astext) {
                ...
                clone = $(el.domobj.outerHTML).empty();
            ...
            if (el.domobj.nodeType === 3) {
              ...
              text += astext ? $(el.domobj).text() : $(el.domobj).outerHTML();
            }
          }
        }
      ...
      removeSelectedMarkup: function() {
        ...
        newRange = new Selection.SelectionRange();
        ...
        my.Selection.rangeObject = newRange;
      ...
      removeFromSelectionTree: function(selectionTree, newRange) {
        ...
        for (i = 0, treeLength = selectionTree.length; i < treeLength; i++) {
          ...
          if (el.selection == 'partial') {
            if (el.domobj.nodeType === 3) {
              ...
              if (firstPartialElement) {
                ...
                if (firstPartialElement.nodeName == el.domobj.nodeName) {
                  // merge the nodes
                  $(firstPartialElement).append($(el.domobj).contents());
            ...
            // eventually set the new range (if not done before)
            if (!newRange.startContainer) {
              adjacentTextNode = my.Dom.searchAdjacentTextNode(
                el.domobj.parentNode,
                my.Dom.getIndexInParent(el.domobj) + 1,
                false,
                {
                  'blocklevel': false
                }
              ...
              if (adjacentTextNode) {
                ...
                newRange.startOffset = newRange.endOffset = my.Dom.getIndexInParent(el.domobj) + 1;
              }
            }
      ...
      splitRangeObject: function(rangeObject, markup) {
        ...
        var splitObject = $(rangeObject.splitObject),
            selectionTree,
            insertAfterObject,
            followUpContainer;
        ...
        $(followUpContainer).insertAfter(insertAfterObject); // attach the followUpContainer right after the insertAfterObject

        // in some cases, we want to remove the "empty" splitObject (e.g. LIs, if enter was hit twice)
        if (rangeObject.splitObject.nodeName.toLowerCase() === 'li' && !my.Selection.standardTextLevelSemanticsComparator(rangeObject.splitObject, followUpContainer)) {
          $(rangeObject.splitObject).remove();
        }
        ...
        if (rangeObject.startContainer) {
          ...
          rangeObject.startOffset = rangeObject.endOffset = my.Dom.getIndexInParent(followUpContainer.get(0));
        }
      ...
      getInsertAfterObject: function(rangeObject, followUpContainer) {
        ...
        for (i = 0; i < rangeObject.markupEffectiveAtStart.length; i++) {
          ...
          if (my.Selection.canTag1WrapTag2($(el).parent()[0].nodeName, followUpContainer[0].nodeName)) {
            return el;
          }
        }
      ...
      splitRangeObjectHelper: function(selectionTree, rangeObject, followUpContainer, inBetweenMarkup) {
        if (!followUpContainer) {
          console.warn(this, 'no followUpContainer, no inBetweenMarkup, nothing to do...');
        }

        var fillUpElement = this.getFillUpElement(rangeObject.splitObject),
            splitObject = $(rangeObject.splitObject),
            startMoving = false,
            el,
            i,
            completeText,
        ...
        if (selectionTree.length > 0) {
          ...
          for (i = 0, treeLength = selectionTree.length; i < treeLength; ++i) {
            ...
            if ((el.selection === 'none' && startMoving === false) || (el.domobj && el.domobj.nodeType === 3 && el === selectionTree[(selectionTree.length - 1)] && el.startOffset === el.domobj.data.length)) {
              ...
              } else if (Dom.isSplitObject(followUpContainer[0])) {
                if (fillUpElement) {
              ...
              if (el.selection !== 'none') { // before cursor, leave this part inside the splitObject
                ...
                if (el.domobj && el.domobj.nodeType === 3 && el.startOffset !== undefined) {
                  ...
                  if (el.startOffset > 0) { // first check, if there will be some text left in the splitObject
                    ...
                    parent = $(el.domobj).parent();
                    if (my.Dom.isSplitObject(parent[0])) {
                      if (fillUpElement) {
                  ...
                  if (completeText.length - el.startOffset > 0) {
                    ...
                    mirrorLevel.eq((i)).remove();
                  } else if (Dom.isBlockLevelElement(followUpContainer[0])) {
                ...
                if (el.selection === 'none' && startMoving === true) {
                  ...
                  jqObj = $(el.domobj).remove();
                }
              }
            }
          }
        } else {
          console.error(this, 'can not split splitObject due to an empty selection tree');
        }
        ...
        if (splitObject.contents().length === 0 && my.Dom.isSplitObject(splitObject[0]) && fillUpElement) {
          splitObject.html(fillUpElement);
        }

        if (followUpContainer.contents().length === 0 && my.Dom.isSplitObject(followUpContainer[0]) && fillUpElement) {
          followUpContainer.html(fillUpElement);
        }
      },
      ...
      getSplitFollowUpContainer: function(rangeObject) {
        ...
        case 'h6':
          ...
          lastObj = $(rangeObject.splitObject).textNodes(':not(.aloha-cleanme)').last()[0];
          ...
          if (lastObj && rangeObject.startContainer === lastObj && rangeObject.startOffset === lastObj.length) {
            returnObj = $('<p></p>');
            inside = $(rangeObject.splitObject.outerHTML).contents();
        ...
        case 'li':
          ...
          if (rangeObject.startContainer.nodeName.toLowerCase() === 'br' && $(rangeObject.startContainer).hasClass('aloha-ephemera')) {
            returnObj = $('<p></p>');
            inside = $(rangeObject.splitObject.outerHTML).contents();
            returnObj.append(inside);
            return returnObj;
          ...
          if (!rangeObject.splitObject.nextSibling && $.trim($(rangeObject.splitObject).text()).length === 0) {
            returnObj = $('<p></p>');
            return returnObj;
          }
          break;
        ...
        return $(rangeObject.splitObject.outerHTML);
      },
