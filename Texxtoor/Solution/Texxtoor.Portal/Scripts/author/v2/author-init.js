var AUTHOR = (function (my) {
  ...
  my.initializeEditor = function (baseUrl, docId, chapterId, editorTitle, uploadServiceUrl, ribbonUrl) {
    ...
    this.serviceUrl = {
      ...
      imageUploadByService: this.uploadServiceUrl,
  ...
  my._attachEvents = function () {
    ...
    shortcut.add("Ctrl+B", function () {
      $('#sn-' + $this.snippetId).htmlarea('bold');
    });
    shortcut.add("Ctrl+I", function () {
      $('#sn-' + $this.snippetId).htmlarea('italic');
    });
    shortcut.add("Ctrl+U", function () {
      $('#sn-' + $this.snippetId).htmlarea('underline');
    });
    ...
    $(document).on('paste', '.editableSection, div.editor, .editableCaption', function (e) {
      ...
      if (window.clipboardData && window.clipboardData.getData) { // IE
        ...
        pastedText = clp.getData('text/plain');
      }
    ...
    $(document).on('keyup', ".canMoveCaret", function (e) {
      ...
      if ((e.keyCode == 8 || e.keyCode == 46) && !e.shiftKey && !e.ctrlKey) {
        ...
        // check content, diff bhvr if empty
        switch (elm.data('editor')) {
        case "HtmlEditor":
        case "TableEditor":
          var content = $(elm).htmlarea('toHtmlString');
          if (content.length > 0 && content[0] == '<') {
            content = $(content).text();
          }
          if (content.length == 0) {
            $this.deleteCommand(elm.data('item'));
            return true;
          }
          break;
        }
    ...
    // special popup functions / entity support
    $(document).on('click', '[data-command="insert"][data-action="entities"] span[title]', function() {
      // <span title="copyright sign" data-entity="#169">&copy;</span>
      var entity = '<span>&' + $(this).data('entity') + ';</span>';
      var elm = $('.editableByTexxtoor[data-item=' + $this.snippetId + ']');
      switch (elm.data('editor')) {
        case 'HtmlEditor':
        case 'TableEditor':
          $(elm).htmlarea('pasteHTML', $(entity).html());
          break;
        case 'SectionEditor':
          var sel, range;
          if (window.getSelection) {
            ...
            if (sel.getRangeAt && sel.rangeCount) {
              ...
              range.insertNode(document.createTextNode($(entity).html()));
            }
          } else if (document.selection && document.selection.createRange) {
            document.selection.createRange().text = $(entity).html();
          }
          break;
        case 'SidebarEditor':
          break;
    ...
    $(document).on('mouseenter', 'img.metaDataText', function () {
      ...
      var infoDiv = $('<div>')
        ...
        .html('<div style="font-weight: bold; font-size: 150%;">Comments</div><small>Click icon for details. Click anywhere to close.</small><br/><br/>')
        .click(function () {
          $(this).remove();
        })
        .appendTo($('body'));
