...
var AUTHOR = (function (my) {
  ...
  my.initSnippetEditor = function (snippetBlock) {
    ...
    switch ($(editable).data("editor")) {
      case "SidebarEditor":
        ...
        $(editable).find('aside').htmlarea({
          onActivate: function (e, sid) {
            if ($this.isLoaded && sid != $this.snippetId) {
              $this.setSnippetId(sid);
            }
          },
          onUndo: function (e, t, d, v) {
            my.undoManager.register(e, $(editable).htmlarea, ['undo', d, v], "Text Operation '" + t + "'");
          }
        }, $this);
        break;
      case "TableEditor":
  ...
  my.insertCommand = function (type, variation, data) {
    ...
    if (($("#sn_block-" + id + " .editor").html() == "<p></p>" || $("#sn_block-" + id + " .editor").html() == "<p>&nbsp;</p>")) {
      ...
      my.scrollManager.setScrollPosition(my.scrollManager.scrollPosition);
      my.caretManager.setEditorCaret();
      return;
    }
    my.setStatusBar("Snippet inserting");
    $.ajax({
      url: my.serviceUrl.insertSnippetByService,
      data: JSON.stringify({
        documentId: my.documentId,
        chapterId: my.chapterId,
        id: id,
        type: type,
        variation: variation,
        data: data
      ...
      success: function (jsonData) {
        ...
        my.setStatusBar("Snippet Inserted..." + idata.msg);
        var t = new Date().now;
        widget.snippetObj = idata.snippet;
        if (type == "chapter") {          
          my._clearEditor();
          my.chapterId = idata.relocateTo;
          my.updateTableOfContent();
          my._appendElementForChapter(widget.getWidgetHtml(), $("#sn_block-" + id), type); // id is current element before inserting 
          my.resize();
          my.scrollManager.setScrollPosition(my.scrollManager.scrollPosition);
          var indexOfLastChapter = my.chapterIdsList.indexOf(oldChapterId);
          if (indexOfLastChapter == my.chapterIdsList.length - 1)
            my.chapterIdsList.push(my.chapterId);
          else
            my.chapterIdsList.splice(indexOfLastChapter + 1, 0, my.chapterId);
          my.setChapterButtons();
        } else {
          my.snippetId = idata.id;
          if (type == "listing")
            widget.snippetObj.snippetCounter = my.getNumberOfSnippet("listingEditor");
          if (type == "table")
            widget.snippetObj.snippetCounter = my.getNumberOfSnippet("tableEditor");
          if (type == "img")
            widget.snippetObj.snippetCounter = my.getNumberOfSnippet("editableImage");
          widget.serviceUrl = my.serviceUrl.imagePath;
          var $elm = my._appendElement(widget.getWidgetHtml(), $("#sn_block-" + id)); // id is current element before inserting 
          setTimeout(function () { my.initSnippetEditor($elm); }, 1);
          my.resize();
          my.scrollManager.setScrollPosition($this.scrollManager.scrollPosition);
          $elm.find('.editor').focus();
        }
        my.AutoSetSnippetEditorButtons();
        my.updateWidgetTools();
        my.undoManager.register(null, my.insertCommand, [type, variation, data, null], "Insert " + type, null, null, null, "");
      },
      error: function (idata) {
        my.setStatusBar("Not inserting...");
        my.hideLoader();
      }
    });
  };
  my.deleteCommand = function (sid) {
    ...
    $.ajax({
      ...
      success: function (idata) {
        ...
        var href = $("#tocpane li:last").prev().find("a").attr('href');
        my.updateTableOfContent();
        my.setStatusBar(data.msg);
        // update content store and remove physically
        //$this.loadSnippetStructure(function (idata) {
        if (isChapterDeleted) {
          my.removeAndGetNewSnippet();
          //$this.setCurrentChapter(href);
          //$this.scrollManager.setScrollPosition(0);
          return;
        } else {
          $('#sn_block-' + sid).fadeOut(200, function () {
            ...
            my.scrollManager.setScrollPosition(my.scrollManager.scrollPosition);
            my.caretManager.setEditorCaret();
            my.refreshSnippetCounter("listingEditor");
            my.refreshSnippetCounter("tableEditor");
            my.refreshSnippetCounter("editableImage");
          });
        }
        var type = $('#sn_block-' + sid).find('.editableByTexxtoor').data('editor');
        my.undoManager.register(null, my.insertCommand, [type, null, null], "Delete " + type);
      },
      error: function (data) {
        my.setStatusBar("Not deleted..." + data.msg);
        // let's reappear to show user that something went wrong
      }
    });
  };
  ...
  my.pasteHtmlAtCaret = function (html, r) {
    ...
    if (editable.length == 1) {
      $(editable).htmlarea('pasteHTML', html, r);
    }
  };
