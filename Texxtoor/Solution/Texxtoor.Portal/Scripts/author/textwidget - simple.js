(function ($) {
  ...
  txxtrEditWidget.fn = txxtrEditWidget.prototype = {
    ...
    Init: function (elem, options) {
      ...
      if ($this.editor) {
        ...
        priv.attachEditorEvents($this);

        if ($(elem).is(".tableEditor")) {
    ...
    execCommand: function (a, b, c) {
      var $this = this;
      $this.focus();
      // save old status to undo stack, if not in an undo
      if ($.isFunction($this.opts.onUndo) && a !== 'undo' && a !== 'redo') {
        $this.opts.onUndo($this.editor.parent(), a, true, $this.toHtmlString());
      }
      // invoke command
      document.execCommand(a, b || false, c || null);
      // change is monitored in AUTHOR and used to save
      $this.editor.trigger('change');
    },
    ec: function (a, b, c) {
      this.execCommand(a, b, c);
    },
    // we encapuslate indeterm, value, and state here
    queryCommandValue: function (a) {
      return {
        state: document.queryCommandState(a),
        indeterm: document.queryCommandIndeterm(a),
        value: document.queryCommandValue(a)
      };
    },
    qc: function (a) {
    ...
    getSelectedHTML: function () {
      ...
      var b = "";
      if ($.browser.msie) {
        s = this.getRange().htmlText;
      } else {
        var elem = this.getRange().cloneContents();
        s = $("<div/>").append($(elem)).html();
      }
      while (s.length > 1 && s[s.length - 1] == ' ') {
        s = s.substring(0, s.length - 1);
        b += " ";
      }
      return { html: s, space: b };
    },
    ...
    __isOrContainsNode: function (ancestor, descendant) {
      var node = descendant;
      while (node) {
        if (node === ancestor) {
          return true;
    ...
    pasteHTML: function (html, r) {
      ...
      if (window.getSelection) {
        ...
        r.deleteContents();
        var repl = html && html.charAt(0) === '<' ? $(html).get(0) : document.createTextNode(html);
        r.insertNode(repl);
        if (html.slice(-1) == ' ') {
      ...
      $this.focus();
      if ($.isFunction(this.opts.onUndo)) {
        this.opts.onUndo(this.editor.parent(), "paste");
      }
      AUTHOR.deferredSave(this.editor);
    },
    ...
    cut: function () {
      if ($.browser.msie) {
        ...
        AUTHOR.clipboard = this.getSelectedHTML().html;
    ...
    insertUnorderedList: function () { this.ec("insertUnorderedList"); },
    ...
    indent: function () {
      ...
      var parent = $($(this.getRange())[0].startContainer.parentElement).parents('li');
      if (parent.length > 0) {
        this.ec("indent");
      }
    },
    ...
    activateEditor: function () {
      if ($.isFunction(this.opts.onActivate)) {
    ...
    onUndoEditor: function () {
      if (typeof (this.opts.onUndo) === "function") {
        this.opts.onUndo(this.snippetId);
      }
    },
    onRedoEditor: function () {
      if (typeof (this.opts.onRedo) === "function") {
  ...
  var priv = {
    attachEditorEvents: function ($that) {
      //$(document).find('*').removeAttr('contenteditable');
      $($that.editor).attr('contenteditable', $($that.editor).data('content-editable'));
      $that.editor
        .on('focus', function () {
          var elm = $(this);
          ...
          var elm = $(this);
          AUTHOR.deferredSave(elm);
        }).on('keyup', function (e) {
          var elm = $(this);
          if (e.which < 33 || e.which > 40) {
            AUTHOR.deferredSave(elm);
          }
          if (e.which == 46 && $that.html() == '') {
            var id = $(elm).data('item');
            AUTHOR.deleteCommand(id);
          }
        });
      var fn = function (e) {
        $that.activateEditor();
      ...
      var fnmu = function (e) {
        if ($(e.target).parents('.equationContainer').length > 0) {
          $that.equation = $(e.target).parents('.equationContainer');
        } else {
          $that.range = $that.getRange();
        }
      };
      $that.editor.on('click keydown', fn);
      $that.editor.on('mouseup', fnmu);
      $that.editor.on('paste', function (e) {
        return true;
      });
      // attach undo redo keys to inform the external undo manager
      var prevKey = null;
      $that.editor.on('keydown', function (e) {
        var thisKey = e.which;

        if (prevKey && prevKey == 17) {
          if (thisKey == 90) $that.onUndoEditor();
          else if (thisKey == 89) $that.onRedoEditor();
        }
        prevKey = thisKey;
      });
    },
