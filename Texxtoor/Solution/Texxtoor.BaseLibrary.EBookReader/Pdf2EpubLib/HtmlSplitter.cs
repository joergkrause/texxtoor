...
namespace Texxtoor.BaseLibrary.Pdf2Epub
{
  public class HtmlSplitter
  {
    ...
    public void SplitFile( ConversionOptions options )
    {
      ...
      _WorkingDir = GetWorkingDir( _CurrentOptions.HtmlFilename );
      _HtmlFooter = _BodyEndTag;
      ProcessFile( _CurrentOptions.HtmlFilename );
    }
    ...
    private string ProcessHtmlPage( string page )
    {
      ...
      if ( IsDocumentOutline( page ) == true )
      {
        ProcessDocumentOutline( page );
        return string.Empty;
      }
      else
      {
    ...
    {
      foreach ( int i in ints.OrderByDescending( i => i ) )
      {
        page = page.Insert( i, "\"" );  
      }
      return page;
    ...
    private bool IsDocumentOutline( string page )
    {
      if ( page.Length < _DocumentOutline.Length )
      {
        return false;
      }
      string part = page.Substring( 0, _DocumentOutline.Length ).ToLower();
      return ( part == _DocumentOutline );
    }

    private void ProcessDocumentOutline( string page )
    {
      NcxCreator creator = new NcxCreator( "title", "author" );
      MatchCollection matchCollection = Regex.Matches( page, "<A href=(?<href>.*?)>(?<title>.*?)</A>" );
      foreach ( Match match in matchCollection )
      {
        string href = match.Groups[ "href" ].Value;
        string title = match.Groups[ "title" ].Value;

        bool isPrintable = CheckIsPrintable( title );
        if ( isPrintable == true )
        {
          href = href.Trim( new char[] { '"' } );
          href = GetNewHref( href );
          creator.AddNavPoint( title, href );
        }
      }
      string filename = Path.Combine( _WorkingDir, "oebps.ncx" );
      creator.SaveDocument( filename );
    }
    ...
    private string GetNewHref( string href )
    {
      ...
      if (( parts.Length == 2 ) && ( _ReferenceList.Keys.Contains( parts[1] )))
      {
