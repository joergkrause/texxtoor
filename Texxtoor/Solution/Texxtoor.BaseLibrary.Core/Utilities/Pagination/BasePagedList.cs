...
namespace Texxtoor.BaseLibrary.Core.Utilities.Pagination {
  ...
  public abstract class BasePagedList<T> : List<T>, IPagedList<T> {
    ...
    internal protected BasePagedList(int index, int pageSize, int totalItemCount, Expression<Func<T, bool>> filter, string order, bool desc) {
    ...
    public int FilterItemCount { get; protected set; }
