...
namespace Texxtoor.BaseLibrary.Core.Utilities {
  /// <summary>
  ...
  public static class StorageSerializer {
    ...
    private static readonly Dictionary<string, IEnumerable<PropertyInfo>> PropertyLists =
      new Dictionary<string, IEnumerable<PropertyInfo>>();

    private static readonly Dictionary<string, IEnumerable<PropertyInfo>> ChecksumLists =
      new Dictionary<string, IEnumerable<PropertyInfo>>();

    [ThreadStatic] private static List<Type> _knownTypes;

    private static IList<Type> _buildInTypes = new[]
                                                 {
                                                   typeof (string),
                                                   typeof (decimal),
                                                   typeof (float),
    ...
    [ThreadStatic] private static Dictionary<object, int> _seenObjects;
    [ThreadStatic] private static List<object> _loadedObjects;
    [ThreadStatic] private static List<string> _propertyIds;

    [ThreadStatic] private static Stack<List<object>> _loStack;
    [ThreadStatic] private static Stack<Dictionary<object, int>> _soStack;
    [ThreadStatic] private static Stack<List<Type>> _ktStack;
    [ThreadStatic] private static Stack<List<string>> _piStack;
    [ThreadStatic] private static bool _isChecksum;
    [ThreadStatic] public static bool IgnoreIds;
    ...
    public static string GetChecksum(object item) {
      ...
      var isChecksum = StorageSerializer.IsChecksum;
      StorageSerializer.IsChecksum = true;
      var toBytes = StorageSerializer.Serialize(item);
      StorageSerializer.IsChecksum = isChecksum;
      ...
      lock (PropertyLists) {
        ...
        if (!IsChecksum) {
          if (!PropertyLists.TryGetValue(itm.AssemblyQualifiedName, out ret)) {
            ret =
              itm.GetProperties(BindingFlags.Instance | BindingFlags.Public).Where(
                p =>
                p.GetCustomAttributes(typeof (DoNotSerialize), false).Count() == 0 &&
                !(p.GetIndexParameters().Count() > 0) && (p.GetSetMethod() != null)).ToArray();
    ...
    private static IEnumerable<FieldInfo> GetFieldInfo(Type itm) {
      lock (FieldLists) {
        ...
        if (FieldLists.ContainsKey(itm))
          ...
          ret =
            FieldLists[itm] =
            itm.GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.SetField).Where(
              p => p.GetCustomAttributes(typeof (DoNotSerialize), false).Count() == 0).ToArray();
        }
    ...
    private static object DeserializeObject(BinaryReader reader, Type itemType = null, object instance = null) {
      ...
      if (tpId != 0xffff || itemType == null)
        itemType = (tpId > 0xff00) ? _buildInTypes[0xfffd-tpId] : _knownTypes[tpId];
    ...
    private static bool IsSimpleType(Type tp) {
      return tp.IsPrimitive || tp == typeof (DateTime) || tp == typeof (TimeSpan) || tp == typeof (string) || tp.IsEnum ||
             tp == typeof (Guid) || tp == typeof (decimal);
    }
    ...
    private static ushort GetTypeId(Type tp) {
      var tmp = _buildInTypes.IndexOf(tp);
      var tpId = (tmp < 0) ? _knownTypes.IndexOf(tp) : 0xfffd - tmp;
