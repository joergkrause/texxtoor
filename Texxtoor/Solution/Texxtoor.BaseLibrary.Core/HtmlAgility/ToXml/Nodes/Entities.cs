...
namespace Texxtoor.BaseLibrary.Core.HtmlAgility.ToXml.Nodes
{
    ...
    public class Entities
    {
        ...
        private static readonly Dictionary<string, string> _full;
        private static Dictionary<string, char> _fullChar;
        ...
        static Entities()
        {
            ...
            _base = LoadEntities("Texxtoor.BaseLibrary.Core.HtmlAgility.ToXml.Nodes.entities-base.txt");
            _baseByVal = ToCharacterKey("Texxtoor.BaseLibrary.Core.HtmlAgility.ToXml.Nodes.entities-base.txt");
            _full = LoadEntities("Texxtoor.BaseLibrary.Core.HtmlAgility.ToXml.Nodes.entities-full.txt");
            _fullByVal = ToCharacterKey("Texxtoor.BaseLibrary.Core.HtmlAgility.ToXml.Nodes.entities-full.txt");
        ...
        private static Dictionary<string, string> LoadEntities(string resourceName)
        {
            _fullChar = new Dictionary<string, char>();
            ...
            using (StreamReader reader = new StreamReader(stream))
            {
                ...
                while (!string.IsNullOrEmpty(line))
                {
                    ...
                    if (!string.IsNullOrEmpty(line))
                    {
                        ...
                        _fullChar.Add(parts[0], (char)Convert.ToInt32(parts[1], 16));
                        entities.Add(parts[0], "" + Convert.ToInt32(parts[1],16));
                    }
                }
            }
        ...
        private static Dictionary<char, string> ToCharacterKey(string resourceName)
        {
            ...
            using (StreamReader reader = new StreamReader(stream))
            {
                ...
                while (!string.IsNullOrEmpty(line))
                {
                    ...
                    {
                        parts = line.Split('=');
                        char key=(char)Convert.ToInt32(parts[1], 16);
                        if (!outMap.ContainsKey(key))
                        {
                            outMap.Add(key, ""+Convert.ToInt32(parts[1],16));
