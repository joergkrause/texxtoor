...
namespace Texxtoor.BaseLibrary.Core.HtmlAgility.StyleParser {
  ...
  public class CssParser {
    ...
    /// </remarks>
    public IList<StyleObject> ParsedStyles {
      get { return p.ParsedStyles; }
    }

    /// <summary>
    /// Write the parsed styles back into the source string.
    /// </summary>
    public void WriteStylesToSource() {
      _source = String.Empty;
      StringBuilder sb = new StringBuilder();
      foreach (var style in ParsedStyles) {
        sb.AppendFormat("{0} {{", style.SelectorName);
        sb.AppendLine();
        foreach (DictionaryEntry rule in style.Styles) {
          sb.AppendFormat("{0} : {1};{2}", rule.Key, rule.Value, Environment.NewLine);
        }
        sb.Append("}");
        sb.AppendLine();
      }
