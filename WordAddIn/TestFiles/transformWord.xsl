...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                exclude-result-prefixes="xs msxsl scr">
  ...
  <xsl:template match="h5" mode="h5">
    ...
    <Element Type="Section" Level="5">
      <xsl:attribute name="Name">
        <xsl:value-of select="normalize-space(ancestor::p/following-sibling::p[@class='FigureCaption']/span[last()]/text())" />
      </xsl:attribute>
      <xsl:text>&#xa;</xsl:text>
      <xsl:value-of select="normalize-space(node()/following-sibling::span[last()]/text())" />
      <xsl:apply-templates mode="Body"/>
  ...
  <xsl:template match="p" mode="Body">
    <xsl:choose>
      <xsl:when test="node()/following-sibling::img">
        <xsl:apply-templates />
      </xsl:when>
      ...
      <xsl:when test="@class='SideBarHeader'">
        ...
        <xsl:apply-templates mode="SideBar" select="following-sibling::p[@class='SideBarContent']" />
        <!--<p>
      ...
      <xsl:when test="@class='MsoListParagraphCxSpFirst'">
        <xsl:variable name="firstPart" select="number(normalize-space((node()/ancestor-or-self::span)[1]/text()))" />
        <xsl:choose>
          <xsl:when test="contains('1234567890', $firstPart)">
            <xsl:text disable-output-escaping="yes">
              &lt;Element Type="Text"&gt;
              &lt;ol&gt;
            </xsl:text>
        ...
        <li>
          <xsl:apply-templates select="node()/following-sibling::span[last()]"/>
        </li>
      </xsl:when>
      ...
      <xsl:when test="@class='MsoNormal'">
        <xsl:variable name="content" select="normalize-space(node()/text())" />
        <xsl:if test="string-length($content) > 1">
          <xsl:text>&#xa;</xsl:text>
          <Element Type="Text">
            <p>
  ...
  <xsl:template match="p[@class='SideBarContent']" mode="SideBar" >
    <xsl:choose>
      <!-- First Content Element, followed by another -->
      <xsl:when test="./preceding-sibling::p[@class='SideBarHeader'] and ./following-sibling::p[1][@class='SideBarContent']">
        <xsl:text disable-output-escaping="yes">&lt;aside&gt;</xsl:text>
        <p>
          <xsl:value-of select="./span"/>
        </p>
      </xsl:when>
      <!-- Content Element, preceded and followed by another -->
      <xsl:when test="./preceding-sibling::p[@class='SideBarContent'] and ./following-sibling::p[1][@class='SideBarContent']">
        <p>
          <xsl:value-of select="./span"/>
        </p>
      </xsl:when>
      <!-- Last Content Element, preceded by another -->
      <xsl:when test="./preceding-sibling::p[@class='SideBarContent'] and not(./following-sibling::p[1][@class='SideBarContent'])">
        <p>
          <xsl:value-of select="./span"/>
        </p>
        <xsl:text disable-output-escaping="yes">&lt;/aside&gt;</xsl:text>
      ...
      <!-- Single Content Element, non before and after -->
      <xsl:when test="./preceding-sibling::p[@class='SideBarContent'] and not(./following-sibling::p[1][@class='SideBarContent'])">
        <aside>
          <p>
            <xsl:value-of select="./span"/>
  ...
  <xsl:template match="img">
    ...
    <Element Type="Image">
      <xsl:attribute name="Name">
        <xsl:variable name="caption" select="normalize-space(ancestor::p/following-sibling::p[@class='FigureCaption']/span[last()]/text())" />
        <xsl:if test="starts-with($caption, ': ')">
          <xsl:value-of select="substring($caption, 3)"/>
        </xsl:if>
        <xsl:if test="not(starts-with($caption, ': '))">
